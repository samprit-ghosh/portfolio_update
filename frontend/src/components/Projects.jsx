...
const Projects = () => {
    ...
    return (
        <section id="projects" className="py-28 relative overflow-hidden" style={{ background: 'var(--bg-main)' }}>
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="text-center mb-16 md:mb-20">
                    ...
                    <p className="mt-4 max-w-xl mx-auto text-sm md:text-base" style={{ color: 'var(--text-main)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10" style={{ perspective: '1200px' }}>
                    {projects.map((project, i) => (
                        <motion.div
                            ...
                            style={{ transformStyle: 'preserve-3d', background: 'var(--bg-card)', border: '1px solid var(--border-subtle)' }}
                        >
                            {/* Image */}
                            ...
                            <div className="p-6">
                                <div className="flex items-start justify-between gap-3 mb-2">
                                    <h3 className="text-lg font-bold transition-colors duration-300" style={{ color: 'var(--text-main)' }}>
                                        {project.title}
                                    </h3>
                                    {project.category && (
                                        <span className="shrink-0 px-2.5 py-1 rounded-lg text-xs font-bold"
                                            style={{ background: 'var(--primary-glow)', color: 'var(--primary)', border: '1px solid var(--border-subtle)' }}>
                                            {project.category}
                                        </span>
                                    )}
                                </div>
